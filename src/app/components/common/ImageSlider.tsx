...
export default function ImageSlider({
  ...
  return (
    <div className={`relative overflow-hidden ${className}`}>
      {/* 이미지 슬라이더 컨테이너 */}
      <div 
        className={`flex transition-transform duration-300 ease-in-out ${isModal ? 'h-full' : ''}`}
        style={{ transform: `translateX(-${currentImageIndex * 100}%)` }}
      >
        {images.map((image, index) => (
          <div
            key={index}
            className={`w-full flex-shrink-0 ${isModal ? 'h-full flex items-center justify-center' : ''}`}
          >
            <img
              src={image}
              alt={`Image ${index + 1}`}
              ...
              decoding="async"
              className={`image-high-quality ${isModal 
