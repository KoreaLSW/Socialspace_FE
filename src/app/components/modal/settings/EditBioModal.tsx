...
export default function EditBioModal({ open, onClose }: EditBioModalProps) {
  const { user } = useCurrentUser();
  const initialBio = useMemo(() => (user?.bio as string) || "", [user?.bio]);
  ...
  const validate = (value: string) => {
    ...
    if (v.length > maxLength)
      return `자기소개는 ${maxLength}자 이하여야 합니다.`;
    return null;
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      >
        ...
        <div className="space-y-2">
          <textarea
            ref={textareaRef}
            className="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-gray-900 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white min-h-[120px] resize-y"
