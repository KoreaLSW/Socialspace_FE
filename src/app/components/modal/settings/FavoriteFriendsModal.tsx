...
import { X, Heart, Users, UserMinus } from "lucide-react";
import { useFavorites, useFavoriteActions } from "@/hooks/useFavorites";
import UserAvatar from "@/app/components/common/UserAvatar";

interface FavoriteFriendsModalProps {
  open: boolean;
  onClose: () => void;
}

export default function FavoriteFriendsModal({
  open,
  onClose,
}: FavoriteFriendsModalProps) {
  ...
  const { favorites, pagination, isLoading, mutate } = useFavorites(
    currentPage,
    20
  );
  const { removeFavorite, isLoading: isRemoving } = useFavoriteActions();

  const handleRemoveFavorite = async (userId: string, nickname: string) => {
    if (confirm(`${nickname}님을 친한친구에서 제거하시겠습니까?`)) {
      try {
            ...
            <div className="space-y-1">
              {favorites.map((favorite) => (
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-2 text-sm text-gray-500 dark:text-gray-400">
                        ...
                        <span>•</span>
                        <span>
                          팔로워 {favorite.followers_count.toLocaleString()}명
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <button
                    onClick={() =>
                      handleRemoveFavorite(favorite.id, favorite.nickname)
                    }
                    disabled={isRemoving}
